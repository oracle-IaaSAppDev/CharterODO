...
entityname=$(hostname -s)_dbinstance
engine_name=$(hostname -s)_engine
output=$basedir/odoenginedbinstance.json
source $basedir/odomeIds

checkentity="python entity_check.py $entityname omc_oracle_db_instance"
...
if [ $result != "0" ]; then
        ...
        echo "    \"displayName\": \"Database\","     >> $output
...
if [ -e "$odomeIds" ]; then
        echo "odoenginedbinstance_meId"=$meId >> $odomeIds
        echo "odoenginedbinstance_name"=$meName >> $odomeIds
else
